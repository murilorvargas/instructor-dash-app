...
import type { InstructorPricingResponse } from './pricing.types'

export async function getAllPricingsServer(
  ...
  try {
    ...
    const response = await axios.get<InstructorPricingResponse[]>(
      `${apiUrl}/person/${personKey}/instructor/${instructorKey}/instructor_pricings`,
      {
        headers: {
